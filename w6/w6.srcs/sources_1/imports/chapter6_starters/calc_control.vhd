...
type statecode is (IS0,IS1,cnt_up1,cnt_dwn1,push_arg1,IS2,cnt_up2,cnt_dwn2,push_arg2,sub,add,push_op_add,push_op_sub);
...
begin
    ...
    -- toestandfunctie
    process(buttons_in,state)
    begin
        case state is
        when IS0 =>
            if buttons_in(4) = '1' then
                new_state <= IS1;
            else
                new_state <= IS0;
            end if;    
        when IS1 =>
            if buttons_in(4) = '1' then
                new_state <= push_arg1;
            elsif buttons_in(3) = '1' then
                new_state <= cnt_up1;
            elsif buttons_in(2) = '1' then
                new_state <= cnt_dwn1;
            else
                new_state <= IS1;
            end if;
